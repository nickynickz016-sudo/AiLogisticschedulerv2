...
import { ActivityLog, ActionType, EntityType, UserProfile } from '../types';
...
export const ActivityLogView: React.FC<ActivityLogViewProps> = ({
  ...
  }, [logs, selectedUserId, selectedActionType, selectedEntityType, fromDate, toDate, searchQuery]);
  ...
  }, [logs, selectedLogForThread]);

  // Statistics
  const totalDeletions = useMemo(() => logs.filter(l => l.action_type === 'DELETE').length, [logs]);
  const totalModifications = useMemo(() => logs.filter(l => l.action_type === 'UPDATE' || l.action_type === 'ALLOCATE').length, [logs]);
  const uniqueActiveUsers = useMemo(() => new Set(logs.map(l => l.user_name)).size, [logs]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mt-0.5">
                Full System Trail • User Activity Tracker • Connected Event History
              </p>
            </div>
          </div>
        </div>
